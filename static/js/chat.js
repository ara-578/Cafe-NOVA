document.addEventListener('DOMContentLoaded', () => {
  ...
  async function sendMessage(text) {
    ...
    try {
      ...
      removeTyping();
      if (!response.ok) {
        addMessage(data.error || 'Velvet AI could not process that request. Please try again.', 'bot');
        return;
      }

      const reply = typeof data.reply === 'string' ? data.reply.trim() : '';
      addMessage(reply || data.error || 'Velvet AI did not return a reply. Please ask again.', 'bot');
    } catch (err) {
      removeTyping();
